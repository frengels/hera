...
using hera::common_reference;
using hera::common_reference_t;
using std::is_same_v;
using std::void_t;
...
struct Tuple_helper<void_t<common_reference_t<Ts, Us>...>,
                    Tuple<Ts...>,
                    Tuple<Us...>>
{
...
{
    {
        ...
        static_assert(
            is_same_v<common_reference_t<B&, D const&, D&>, B const&>);
    ...
    static_assert(
        is_same_v<common_reference_t<int const volatile&&, int volatile&&>,
                  int const volatile&&>);
    static_assert(
        is_same_v<common_reference_t<int&&, int const&, int volatile&>,
                  int const volatile&>);

    static_assert(is_same_v<common_reference_t<int(&)[10], int(&&)[10]>,
                            int const(&)[10]>);
    static_assert(
...
{
    {
        ...
        static_assert(is_same_v<common_reference_t<moveonly2 const&, moveonly>,
                                moveonly>);
        static_assert(is_same_v<common_reference_t<moveonly const&, moveonly2>,
                                moveonly>);
    }
...
{
    static_assert(!is_trait<common_reference<volatile Tuple<short>&,
                                             const Tuple<int, short>&>>);
}
