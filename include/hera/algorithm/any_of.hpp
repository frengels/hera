...
struct any_of_fn
{
    ...
    constexpr auto operator()(I first, S last, PredMeta pm) const noexcept
        -> decltype(invoke<false>(std::move(first),
                                  std::move(last),
                                  std::move(pm)))
    {
        return invoke<false>(std::move(first), std::move(last), std::move(pm));
    }
    ...
    template<hera::forward_iterator I,
             ...
             hera::metafunction     PredMeta>
    constexpr auto operator()(I first, S last, PredMeta pm) const noexcept
        -> decltype(invoke<false>(std::move(first), std::move(last)))
    {
        return invoke<false>(std::move(first), std::move(last), std::move(pm));
    }
    ...
    constexpr auto operator()(R&& r, Pred) const noexcept
        -> decltype(invoke<false>(hera::begin(r),
                                  hera::end(r),
                                  hera::type_identity<Pred>{}))
    {
        return invoke<false>(
    ...
    constexpr auto operator()(R&& r, PredMeta pm) const noexcept
        -> decltype(invoke<false>(hera::begin(r), hera::end(r), std::move(pm)))
    {
        return invoke<false>(hera::begin(r), hera::end(r), std::move(pm));
    }
};

constexpr auto any_of = any_of_fn{};
} // namespace hera